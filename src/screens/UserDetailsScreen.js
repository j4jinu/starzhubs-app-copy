...
import { StyleSheet, Text, Image, View, ScrollView, TouchableOpacity, Modal, Alert, TextInput } from 'react-native';
    ...
    return (
        ...
        <ScrollView style={{ flex: 1, backgroundColor: 'white' }}>
            <View style={styles.container}>
                ...
                {userId !== auth.userId && user.isAdminApproved !== 0 && user.isAdminApproved !== 2 && (
                    <>
                    {isFriends.type === 'sent' && (isFriends.status === 'AdminPending' || isFriends.status === 'Pending') && (
                    <TouchableOpacity style={styles.requestBtn} onPress={cancelRequest} activeOpacity={0.7}>
                        <Icon
                    ...
                    { (isFriends.status === 'Strangers')  && (
                     <TouchableOpacity style={styles.requestBtn} onPress={handleRequest} activeOpacity={0.7}>
                        <Icon
                            name="account-plus"
                            size={25}
                            color={'white'}
                    ...
                    <TouchableOpacity style={styles.requestBtn} onPress={handleClickOpenDecline} activeOpacity={0.7}>
                    <Icon
                        name="account-check"
                        size={25}
                        color={'white'}
                    ...
                    {isFriends.type === 'received' && isFriends.status === 'Pending' && (
                    <TouchableOpacity style={styles.requestBtn} onPress={handleRecievedRequest} activeOpacity={0.7}>
                    <Icon
                ...
                <Text style={styles.otherText}>
                    {user.gender},  {calculateAge(user.dob) } yrs, {Moment(user.dob).format('DD/MM/YYYY')}
                </Text>
                ...
                {content === 'T' && <UserTalentSection talents={talents} navigation={props.navigation} />}
                {content === 'M' && <UserMediaSection talents={talents} navigation={props.navigation} />}
            ...
            >
                <View style={{ backgroundColor:'#000000aa', position: 'absolute', bottom: 0, left: 0, right: 0, top: 0}}>
                    <View style={{backgroundColor:'#fff', top:10, padding:20, borderRadius:4, height:'100%'}}>
                    <View style={{flexDirection:'row-reverse', marginBottom:40}}>
                        <TouchableOpacity onPress={()=>setRequestModal(false)} activeOpacity={0.7}>
                            <Icon
                            name="close-circle"
                            size={35}
                            color={'black'}
                    ...
                    <View style={{justifyContent:'center'}}>
                        ...
                        <View style={{marginTop:10}}>
                            <View style={{flexDirection: 'row', marginLeft:20, marginRight:20}}>
                                {talents.map(item=>(
                                    <TouchableOpacity 
                                        key={item._id} 
                                        style= {checked === item._id?styles.categoryItemActive:styles.categoryItem}
                                        onPress={()=>{setChecked(item._id); handleToggle(item._id) }}
                                    >
                        ...
                        <View>
                            ...
                            <View style={{marginLeft:25, marginRight:25, marginTop:10}}>
                                ...
                                <Text style={{ fontSize: 14, color: 'firebrick', alignSelf: 'flex-start',marginTop:5}}>{errors.notes}</Text>
                                }    
                                <View style={{marginTop:20, width:'100%', alignItems: 'center'}}>
                                <TouchableOpacity style={{borderRadius:2,backgroundColor:'orange',padding:10, width:'50%',alignItems:'center'}} onPress={handleSubmit}>
