...
const HomeScreen = (props) => {
    ...
    const getRandomUsers = async () => {
        axios.get('https://api.starzhubs.com/api/talent/random')
            .then((response) => {
                // handle success
                    ...
                    {category.map(cat => (
                        <TouchableOpacity style={styles.categoryGrid}>
                            <View style={styles.categoryGridItem}>
                                <Image
                                    style={{ width: '100%', height: 30, resizeMode: 'center', borderRadius: 10 }}
                                    source={{
                                        uri: 'https://static.thenounproject.com/png/1041139-200.png'
                                    }}
                                />
                                <Text>{cat.title}</Text>
                            </View>
